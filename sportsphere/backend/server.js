...
// 4. Connect Databases (PostgreSQL + PostGIS & MongoDB)
connectPostgres();
connectDB();
...
io.use((socket, next) => {
  ...
  if (!token) {
    socket.user = { id: 'user_1', name: 'Vivek Kumar', role: 'ATHLETE' };
    return next();
  }
  ...
  try {
    ...
    socket.user = { id: decoded.sub || decoded.id, name: decoded.name, role: decoded.role || 'ATHLETE' };
    next();
  } catch (err) {
...
// API Routes (Modular Monolith Domain Modules)
...
const handleHealthCheck = (req, res) => {
  res.json({
    status: 'ONLINE',
    service: 'SportSphere Modular Monolith Backend API Service',
...
server.listen(PORT, () => {
  console.log(`🚀 [SportSphere Modular Monolith Server] Running on http://localhost:${PORT}`);
});
