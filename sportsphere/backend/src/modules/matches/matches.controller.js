...
export const createMatch = async (req, res, next) => {
  try {
    const match = await matchesService.createMatchService(req.user.athleteId, req.body);
...
export const joinMatch = async (req, res, next) => {
  try {
    const updatedMatch = await matchesService.joinMatchService(req.params.id, req.user.athleteId);
...
export const leaveMatch = async (req, res, next) => {
  try {
    const result = await matchesService.leaveMatchService(req.params.id, req.user.athleteId);
